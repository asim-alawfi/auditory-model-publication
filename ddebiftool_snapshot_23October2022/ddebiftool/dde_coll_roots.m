function rts=dde_coll_roots(pt,c,varargin)
%% find roots of c*x(t)-res or c*x'(t)-res on interval [0,1]
% (may fail for phase rotations, as it assumes periodicity)
default={'diff',0,'safety',1e-3,'res',0,'relative',true};
options=dde_set_options(default,varargin,'pass_on');
%% find subintervals and values of c*x-res
ic=1:pt.degree:length(pt.mesh);
nint=length(ic)-1;
if options.diff>0
    cxvals0=c*dde_coll_eva(pt.profile,pt.mesh,pt.mesh(1:end-1),pt.degree,'diff',options.diff,'kron',true)-options.res;
    cxvals1=c*dde_coll_eva(pt.profile,pt.mesh,pt.mesh(ic(2:end)),pt.degree,...
        'diff',options.diff,'kron',true,'submesh_limit',1)-options.res;
else
    cxvals_all=c*pt.profile-options.res;
...
ijump=sign(cxvals(end,:).*cxvals(1,[2:end,1]))<0;
