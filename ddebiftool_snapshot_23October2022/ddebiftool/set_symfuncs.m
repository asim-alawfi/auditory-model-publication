...
if directional_derivative
    sys_rhs=@(x,p)dde_sym_rhs_wrap(f,0,x,p(1,1:npar,:),x,p(1,1:npar,:));
else
    sys_rhs=@(x,p)dde_sym_rhs_wrap(f,0,x,p(1,1:npar,:),{},{});
end
maxorder=f('maxorder');
if directional_derivative
    for order=maxorder:-1:1
        sys_dirderi{order}=@(x,p,dx,dp)dde_sym_rhs_wrap(f,order,x,p(1,1:npar,:),dx,dp(1,1:npar,:));
    end
    sys_mfderi={};
else
    for order=maxorder:-1:1
...
if sd_delay
    ntau=f('ntau');
    sys_ntau=@()ntau;
    sys_tau=@(itau,x,p)dde_sym_tau_wrap(f,itau,0,x,p(1,1:npar,:),x,p(1,1:npar,:));
    for order=maxorder:-1:1
