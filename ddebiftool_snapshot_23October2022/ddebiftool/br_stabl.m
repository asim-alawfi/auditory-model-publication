...
for i=irg
    ...
    if options.exclude_trivial && ~isempty(excl) && ~isempty(ev)
        if length(excl)>=length(ev)
            ev=[];
        else
            iex=dde_match_complex(excl(:),ev(:),evdist);
            triv_defect(i)=norm(reshape(ev(iex),[],1)-excl(:),'inf');
            ev(iex)=[];
        end
    end
