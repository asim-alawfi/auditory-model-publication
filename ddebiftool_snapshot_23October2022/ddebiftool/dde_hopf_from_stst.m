...
%%
default={'funcs',[],'method',getfield(df_mthod('stst'),'stability'),...
...
if ~isempty(options.funcs)
    hopf=dde_stst_critical_space(options.funcs,hopf,'output','pini','kind','hopf',pass_on{:});
else
    [freq,hopf,ind]=dde_stst_critical_freq(options.funcs,hopf,pass_on{:});
    hopf.omega=freq;
