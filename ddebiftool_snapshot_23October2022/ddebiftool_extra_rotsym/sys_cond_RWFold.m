function [r,J]=sys_cond_RWFold(p,funcs,ip,pref)
...
p_orig=p;
p_orig.x=p.x(1:dim);
p_orig.parameter=p_orig.parameter(1:ip.rho-1);
if funcs.orig_cond_reference
    userref=pref;
    userref.x=userref.x(1:dim);
    userref.parameter=userref.parameter(1:ip.omega);
...
rphas=Jphas.x'*p.x;
rnorm=sum(p.x(dim+1:end).^2)+p.parameter(ip.rho)^2-1;
Jnorm=p_axpy(0,p,[]);
Jnorm.x(dim+1:end)=2*p.x(dim+1:end);
Jnorm.parameter(ip.rho)=2*p.parameter(ip.rho);
