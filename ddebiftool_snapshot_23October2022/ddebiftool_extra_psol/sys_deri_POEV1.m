function J=sys_deri_POEV1(xx,par,nx,np,v,hjac,...
    ind_beta,ind_period,dim,xtau_ind,funcs,ext_rhs,use_df_deriv)
%% partial derivatives of r.h.s of extended DDE for periodic orbits with extra FM=1
%
...
if length(nx)==1 && isempty(np) && isempty(v)
    ...
    if ~use_df_deriv
        J=sys_drhs_dx_POEV1(nx,xx,par(1:ind_beta-1),par(ind_beta),...
...
elseif isempty(nx) && length(np)==1 && isempty(v)
    ...
    if np<ind_beta
        ...
        if ~use_df_deriv
            J=sys_drhs_dp_POEV1(np,xx,par(1:ind_beta-1),par(ind_beta),...
...
function J=sys_drhs_dx_POEV1(ind,x,p,beta,period,tau,sys_rhs,dim,xtau_ind,sys_deri)
...
function J=sys_drhs_dp_POEV1(ind,x,p,beta,period,ind_tau,sys_rhs,dim,xtau_ind,sys_deri)
%% derivative of rhs of extended DDE for periodic orbits with FM=1 wrt to x(:,i)
xall=x(1:dim,:,:);
x0=xall(:,xtau_ind(1,:),:);
v=x(dim+1:end,xtau_ind(1,:),:);
tau=[0,p(ind_tau)];
