...
btfuncs=set_funcs(...
    ...
    'sys_deri',sys_deri,'x_vectorized',false);%,...
    %'sys_cond',@(p)sys_cond_BT(p,@extract_from_BT,funcs.sys_cond));
btfuncs.get_comp=@extract_from_BT;
btfuncs.userfuncs=funcs;
btfuncs.sys_cond=@(p,pref)dde_sys_cond_collect(funcs,p,pref,{@(p,pref)sys_cond_BT(p)});
btfuncs.sys_cond_reference=true;
