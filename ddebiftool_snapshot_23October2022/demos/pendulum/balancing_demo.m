...
%
% after non-dimensionalization. The parameters |a|, |b| and |tau| are taken
% from a small ellipse around the degenerate point |a=1, b=tau=tau_*| of
% the form
% 
%   a=1+r^6*alpha;
...
% 
% where |tau_star=sqrt(8-6*epsilon)/2| and |epsilon| is the mass ratio
% between cart and pendulum. The rescaled parameters |alpha,beta,gamma| are
% variables on the ellipse of radii |~r^[6,2,4]|. The three parameters are
% parametrized by the spherical angles on the ellipse |phi,psi| via
%
%   alpha=sin(pi/2*phi);
%   beta=cos(pi/2*phi)*cos(2*pi*psi);
%   gamma=cos(pi/2*phi)*sin(2*pi*psi);
%
% Most bifurcations occur in the quadrant 0<=phi<=1, 0.5<=psi<=1.
%
% Reference:
% 
...
ststcond=@(p,pref)dde_stst_lincond(p,xdim,'x','trafo',Rsym);
...
bp_v_sym=@(p,pref)dde_stst_lincond(p,xdim,'v','trafo',zeros(size(Rsym)),...
    'rotation',[0,1],'condprojmat',eye(size(Rsym)));
bp_scale=@(p,pref)dde_scale_cond(p,pref,bp_v_sym,'res',1);
bp_scale_ini=@(p,pref)dde_scale_cond(p,pref,bp_v_sym,'res',0);
...
hopfcond=@(p,pref)dde_stst_lincond(p,xdim,'v','trafo',Rsym,'rotation',[1,2]);
hopf_v_id=@(p,pref)dde_stst_lincond(p,xdim,'v','trafo',zeros(size(Rsym)),...
    'rotation',[0,1],'condprojmat',eye(size(Rsym)));
hopf_scale=@(p,pref)dde_scale_cond(p,pref,hopf_v_id,'res',1);
hopf_scale_ini=@(p,pref)dde_scale_cond(p,pref,hopf_v_id,'res',0);
hopf_phase=@(p,pref)dde_scale_cond(p,pref,hopf_v_id,'res',0,'matrix',kron([0,1;-1,0],eye(xdim)),'ref',true);
...
[shfuncs,hopf,suc]=SetupHopf(funcs,eqs_wbifs,ihopf,'contpar',[ip.a,ip.b],'dir',ip.b,...
    ...
    'stops',{@(p)p(end).omega<0},hopfargs{:});
%% Continue Hopf bifurcations
figure(2);
...
sbvsym=@(p,pref)dde_psol_lincond(p,xdim,'v','trafo',Rsym,'shift',[1,2],...
    'condprojint',linspace(0,0.5,6)');
sbvscale=@(p,pref)dde_scale_cond(p,pref,sbvsym,'res',1);
sbvscale_ini=@(p,pref)dde_scale_cond(p,pref,sbvsym,'res',0);
...
[pdfuncs,pdbr,suc]=SetupPeriodDoubling(funcs,ns_wbifs,ns_bifind(1),...
    ...
    'dir',ip.b,'max_step',[],...
    'minimal_angle',0,'plot_measure',[],'print_residual_info',1,'use_tangent',true,...
    'usercond',{@(p,pref)dde_pd_cond(p,xdim,linspace(0,0.75,4))},'extracolumns','auto')
