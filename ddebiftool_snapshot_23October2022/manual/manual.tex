...
fold (abbreviated \blist{'fold'}) bifurcation point, a periodic
solution point (abbreviated \blist{'psol'}) or a connecting orbit
point (abbreviated \blist{'hcli'}). Furthermore, a point can contain
...
of the \blist{'kind'} filed in the point structure. These constructors
...
,     \begin{bmatrix}
  ...
  \end{bmatrix}
\end{align*}
and the same delays $\tau_1$,\ldots,$\tau_m$.
...
$x^*(t)=x^*_0=\ldots=x^*_m\equiv x^*\in\RR^n$, and
$\tau^*_j(t)\equiv \tau_j(x^*,\ldots,x^*,\eta^*)$ for all $j\geq1$,
with
\[
f(x^*,x^*,\ldots,x^*,\eta^*)=0\mbox{.}
\]
Then the matrices $A_i(t)$ are constant, $A_i(t)\equiv A_i$, and the
vectors $(x^*)'(t-\tau_j(t))B_{i,j}(t)$ consist of zero elements only.  In this case, the
...
\item \label{defsys:num} For initial exploration of equilibria, periodic orbits and
  ...
  side should be provided (see, e.g., \Cref{neuron_sys_rhs}).
\item \label{defsys:symb} Once basic analysis is successful and more advanced tasks such
  ...
  is using the symbolc toolbox and \blist{dde_sym2funcs} (see
  ...
  requires higher-order derivatives of order $3$ or greater, for whch
...
\item If problems or inexplicable discrepancies between results using
  ...
  derivatives manually. The simplest form are directional derivatives,
...
\item For backward compatibility the user may also provide first- and
  second-order derivatives in the form compatible with versions v.~3.1
  and earlier, explained in \Cref{sec:constjac} for constant delays
  and \Cref{sec:sdjac} for state-dependent delays.
\end{enumerate}
...
\item \blist{'directional_derivatives'} (logical, default
  \blist{false}) should derivatives be stored as directional
...
\subsubsection{Derivatives or Jacobians of right-hand side ---
  \texorpdfstring{\blist{sys_deri}}{sys\_deri}, manually provided}\label{sec:constjac}
\begin{quote}
  ...
  The function described in this section is the ``manual'' and most
...
dde_sym2funcs(f,x,par,'sd_delay',delays,...
...
\subsubsection{Jacobian or directional derivatives of right-hand
  side. manually provided}
\label{sec:sdjac}
...
present here the routine for \blist{sys_deri} since it is quite long,
see the MATLAB code \file{sd\_deri.m} in the demo example
\file{sd\_demo}. If the user does not provide a function for the
Jacobians the finite-difference approximation will be used by default.
However, as for constant delays, it is recommended to provide at least
the first order derivatives with respect to the state variables using
analytical formulas. Alternatively, the user may provide directional
derivatives of \blist{sys_rhs}.
...
by an optional argument for \blist{set_funcs}, \blist{sys_cond_reference} (default \blist{false} for first
...
\begin{quote}
  ...
  \blist{true} and inspect the eigenfunctions.
\end{quote}
