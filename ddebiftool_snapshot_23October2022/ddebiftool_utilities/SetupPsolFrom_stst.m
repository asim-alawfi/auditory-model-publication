...
[funcs,hbr,suc]=SetupHopf(funcs,ststbranch,ind,'outputfuncs',true,...
    'contpar',options.contpar,'corpar',options.corpar,...
    'includehopf',options.includehopf,hopfargs{:},pass_on{:});
if suc==0
    varargout=dde_setupoutput(funcs,[],suc,options.outputfuncs);
    return
end
hopf=hbr.point(1);
deg_psol=p_topsol(funcs,hopf,0,options.degree,options.intervals,pass_on{:});
...
funcs=dde_add_cond('SetupPsolFrom_stst',funcs,options);
[psol,suc]=p_correc(funcs,dev_psol,options.contpar,step_cond,per.method.point,1, ...
    dev_psol);
if suc==0
    varargout=dde_setupoutput(funcs,[],suc,options.outputfuncs);
    return;
