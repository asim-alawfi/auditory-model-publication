...
clf
hold on
set(gca, set_one_bif{:},'FontSize',10,'FontWeight','bold')
pltcoh=plot3(1000./coher(:,2),2.^(coher(:,1)/12)-1,6*ones(length(coher)),'x','Color','k','LineWidth',1,'MarkerSize',7);
pltfiss=plot3(1000./fiss(:,2),2.^(fiss(:,1)/12)-1,6*ones(length(fiss)),'x','Color','k','LineWidth',1,'MarkerSize',7);
x_text_fiss =7; 
y_text_fiss =0.31; 
text(x_text_fiss, y_text_fiss, 'fission', 'FontSize', 16, 'Color', 'black','Rotation', 0)
x_text_coh = 15; 
y_text_coh= 0.327;
text(x_text_coh,y_text_coh,'coherence', 'FontSize', 16, 'Color', 'black','Rotation',0)
