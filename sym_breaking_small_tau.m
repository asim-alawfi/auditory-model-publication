...
%%
parbd={'min_bound',[in.PR,1;in.df,0],'max_bound',[in.PR,75; in.df,1],...
    'max_step',[in.PR,0.08; in.df,0.05; 0,0.05],'print_residual_info',1};
...
[funcs_symbrk_tu,symbk_brtu,succ]=SetupPOEV1(funcs_tau2,po_symmetry_tinyTau2,bifpoints_tinytau2(1),...
...
%%
symbk_brtu.parameter.max_step(4)=0.1;
symbk_brtu.parameter.max_step(5)=0.08;
symbk_brtu.parameter.max_step(end)=0.08;
...
save('sym_breaking_small_tau_part1.mat')
%% We continiue the computation with  increasing the resolution to solve the discontinuity 
symbk_brtu2=symbk_brtu;
symbk_brtu2.point(end-1)=p_remesh(symbk_brtu2.point(end-1),6,120);
symbk_brtu2.point(end)=p_remesh(symbk_brtu2.point(end),6,120);
%%
symbk_brtu2.parameter.max_step(4)=0.05;
symbk_brtu2.parameter.max_step(5)=0.05;
symbk_brtu2.parameter.max_step(end)=0.05;
figure(8)
hold on
...
symbk_brtu=br_remove_extracolumns(symbk_brtu);
save('sym_breaking_small_tau_part2.mat')
...
c_A=[1,0,0,0,0,0];
smaxval=0.5;
sympo_uA_extrema=arrayfun(@(p)dde_coll_roots(p,c_A,'diff',1)',asym_brtinyTau2.point,'uniformoutput',false);
...
sympomax_ua=cellfun(@(p,t)max2(ua_eval(p,t)),num2cell(asym_brtinyTau2.point),sympo_uA_extrema);
...
ine.val=ine.t0+1;
sympo0=setfield(asym_brtinyTau2,'point',asym_brtinyTau2.point(theta_cross));
...
%%
pt=asym_brtinyTau2.point(theta_cross);
figure(77)
clf
hold on; grid on
plot(pt.mesh*pt.period,pt.profile(1:2,:),'LineWidth',2)
%%
[mfuncs,mbranch,suc_max]=ChangeBranchParameters(funcs_audi,sympo0,1,'contpar',[ine.PR,ine.df,ine.t0],...
...
mbranch=br_contn(mfuncs,mbranch,100);
%%
clear mbranch1
